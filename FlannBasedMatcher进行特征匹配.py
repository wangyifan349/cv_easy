...
index_params = dict(algorithm=0, trees=5)  # 初始化参数
search_params = dict(checks=50)  # 检索参数
flann = cv2.FlannBasedMatcher(index_params, search_params)  # 初始化FlannBasedMatcher对象
matches = flann.knnMatch(des1, des2, k=2)  # 对图片1和图片2的描述符进行匹配，k=2表示返回最佳的两个匹配结果

# 应用比例测试以获得良好的匹配
good_matches = []
for m, n in matches:
    if m.distance < 0.7 * n.distance:  # 进行比例测试
